...
class DPlayer {
    ...
    constructor (options) {
        ...
        // 用户配置相关，写入 localstorage
        this.user = new User(this);
        // 注册 container 元素
        this.container = this.options.container;

        this.container.classList.add('dplayer'); // 添加 dplayer class
        // 如果没有弹幕
        ...
        // 如果是手机，显示 页面全屏按钮，隐藏音量，截图按钮
        if (isMobile) {
            this.container.classList.add('dplayer-mobile');
        }
        // container 小于 550 弹幕字体 18,开始22
        ...
        this.template = new Template({
            container: this.container, // 目标容器
            options: this.options, // 配置
            index: index, // 当前视频质量索引
            tran: this.tran, // tran 方法 this.tran('Top')
            icons: this.icons // icons.get(type) type 为 options 中定义
        });

        this.video = this.template.video; // video 标签

        this.bar = new Bar(this.template); // 音量，进度等进度条dom事件绑定

        this.bezel = new Bezel(this.template.bezel); // 视频中心动画效果 <span class="dplayer-bezel-icon"></span>

        this.controller = new Controller(this); // controller 注册，主要是 控制条显示隐藏
        ...
        // console.log(this.setting.loop); loop speed ~
        // console.log(this.focus) // undefined 给 document 添加点击事件设置 focus 为 false
        ...
        // play and pause button 播放暂停按钮
        this.paused = true;
        this.template.playButton.addEventListener('click', () => {
            this.toggle(); // 播放暂停
        });

        // 如果是 pc 点击同样进行播放暂停。手机则显示隐藏控制条
        if (!isMobile) {
